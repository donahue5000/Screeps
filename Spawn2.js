...
var Spawn2 = {
    run: function(spawn){
        ...
        var towers = Game.spawns[spawn].room.find(FIND_STRUCTURES, {filter:
        ...
        if (mineCount < 2){
            Game.spawns[spawn].createCreep([
                ...
                MOVE
                ], 'm' + (Game.time), {'role':'mine', 'source':nextSource, 'home':spawn});
        }
        if (haulCount < 1){
            Game.spawns[spawn].createCreep([
                MOVE, MOVE,
            ...
            Game.spawns[spawn].createCreep([
                ...
                MOVE,
                CARRY
                ], 'r' + (Game.time), {'role':'repair', 'home':spawn});
        }else if (xmineCount < 0){
            Game.spawns[spawn].createCreep([
                WORK, WORK, WORK, WORK, WORK,
                MOVE, MOVE
                ], 'xm' + (Game.time), {'role':'xmine', 'home':spawn});
        }else if (xhaulCount < 0){
            Game.spawns[spawn].createCreep([
                MOVE, MOVE, MOVE, MOVE, MOVE,
                CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY
                ], 'xh' + (Game.time), {'role':'xhaul', 'home':spawn});
        }else if (reserverCount < 0){
            Game.spawns[spawn].createCreep([
                MOVE,
                CLAIM, CLAIM
                ], 'res' + (Game.time), {'role':'reserver', 'home':spawn});
        }else if (xbuildCount < 0){
            Game.spawns[spawn].createCreep([
                MOVE, MOVE, MOVE,
                CARRY, CARRY, CARRY,
                WORK, WORK, WORK
                ], 'xb' + (Game.time), {'role':'xbuild', 'home':spawn});
