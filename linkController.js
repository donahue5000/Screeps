let linkController = {
    run: function(thisRoom) {
        ...
        let storeLink = Game.getObjectById(thisRoom.memory.storeLink);
        if (storeLink && storeLink.energy < 767){
            let links = thisRoom.find(FIND_STRUCTURES, {
                filter: (structure) => structure.structureType == STRUCTURE_LINK &&
                structure.id != storeLink.id
            });
            if (links.length > 0){
                for (let link in links){
                    let highLink = links[link];
        ...
        if (Game.cpu.bucket > 9000 && (
            ...
            if (thisRoom.memory.g && thisRoom.memory.h && thisRoom.memory.gh &&
            ...
            if (thisRoom.memory.gh2o && thisRoom.memory.inputX && 
                ...
                Game.getObjectById(thisRoom.memory.xgh2o2).runReaction(Game.getObjectById(thisRoom.memory.gh2o), Game.getObjectById(thisRoom.memory.inputX))
            }
        }
